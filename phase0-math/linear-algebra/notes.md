...
#### Basis Vectors
좌표계에서는 단위벡터라는 특별한 벡터가 있다. 2차원 공간에서 단위벡터는 두 개 존재한다. 각 단위벡터를 상수배 한 벡터들의 합으로 2차원 공간 전체를 표현할 수 있다. 반대로 단위벡터가 되려면 상수배를 한 것들끼리 더했을 때 공간을 모두 표현할 수 있어야 한다는 것이다. 

단순하게 생각해서 2차원 공간에서 [1, 0]과 [0, 1] 두 벡터 쌍은 단위벡터가 될 수 있지만, [1, 0] 과 [2, 0] 은 단위벡터가 될 수 없다. 후자의 경우 아무리 서로 상수배 한 벡터들을 더해봐도 [x, 0] 공간만 표현할 수 있기 때문이다.
